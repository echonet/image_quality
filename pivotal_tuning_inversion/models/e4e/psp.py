...
class pSp(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            ckpt = torch.load(self.opts.checkpoint_path, map_location='cpu', weights_only=True)
            ...
            ckpt = torch.load(self.opts.stylegan_weights, weights_only=True)
    ...
    def __load_latent_avg(self, ckpt, repeat=None):
        ...
        if 'latent_avg' in ckpt:
            self.latent_avg = ckpt['latent_avg'].to(device) #.to(self.opts.device)
        elif self.opts.start_from_latent_avg:
            # Compute mean code based on a large number of latents (10,000 here)
            with torch.no_grad():
                self.latent_avg = self.decoder.mean_latent(10000).to(device) #.to(self.opts.device)
        else:
